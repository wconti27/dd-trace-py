...
def test_rc_activation_capabilities(tracer, remote_config_worker, env_rules, expected):
    env = env_rules.copy()
    with override_env(env), override_global_config(dict(_remote_config_enabled=True, _appsec_enabled=False, api_version="v0.4")):
        rc_config = {"config": {"asm": {"enabled": True}}}
...
def test_rc_activation_check_asm_features_product_disables_rest_of_products(
    ...
    env = env_rules.copy()
    with override_env(env), override_global_config(dict(_remote_config_enabled=True, _appsec_enabled=True, api_version="v0.4")):
